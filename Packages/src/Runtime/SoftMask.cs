...
namespace Coffee.UISoftMask
{
    ...
    public class SoftMask : Mask, ISoftMasking, IMaskable, IMaskingShapeContainerOwner, ISerializationCallbackReceiver
    {
        ...
        public bool renderEveryFrame = false;
        ...
        protected override void OnEnable()
        {
            ...
            if (graphic)
            {
                ...
                graphic.RegisterDirtyVerticesCallback(_setSoftMaskDirty ?? (_setSoftMaskDirty = SetSoftMaskDirty_FromVertices));
        ...
        private void OnBeforeCanvasRebuild()
        {
            switch (maskingMode)
            {
                ...
                case MaskingMode.SoftMasking:
                {
                    ...
                    {
                        // SetSoftMaskDirty();
                        SetSoftMaskDirty_FromTransform();
                    }
        ...
        {
            if (!isActiveAndEnabled || !graphic || !graphic.canvas)
                return baseMaterial;
        ...
        // {
        //     if (isDirty || !this || !isActiveAndEnabled)
        //         return;
        ...
        public void SetSoftMaskDirty()
        {
            ...
            if (Time.frameCount % 60 == 0)
                Debug.Log($"[SoftMask] SetSoftMaskDirty called: {name} frame={Time.frameCount} time={Time.time} playing={Application.isPlaying}");
#endif
            if (!this || !isActiveAndEnabled)
                return;

            // Prevent dirty spam within the same frame.
            if (_lastDirtyFrame == Time.frameCount)
                return;
            ...
            // Propagate to children (keep your behavior)
            for (var i = children.Count - 1; i >= 0; i--)
            {
                if (children[i])
                    children[i].SetSoftMaskDirty();
        ...
        private void SetSoftMaskDirty_FromVertices() => SetSoftMaskDirty_WithReason("DirtyVerticesCallback");
        private void SetSoftMaskDirty_FromTransform() => SetSoftMaskDirty_WithReason("TransformChanged");
        private void SetSoftMaskDirty_FromView() => SetSoftMaskDirty_WithReason("CanvasViewChanged");
        private void SetSoftMaskDirty_FromDim() => SetSoftMaskDirty_WithReason("RectDimensionsChanged");
        private void SetSoftMaskDirty_FromAnim() => SetSoftMaskDirty_WithReason("AnimationProperties");

        private void SetSoftMaskDirty_WithReason(string reason)
        {
#if UNITY_EDITOR
            if (Application.isPlaying && Time.frameCount % 60 == 0)
                Debug.Log($"[SoftMask] Dirty reason={reason} name={name} frame={Time.frameCount}");
#endif
            SetSoftMaskDirty();
        }
        ...
        {
            if (graphic && graphic.IsInScreen())
                return true;
            if (_shapeContainer && _shapeContainer.IsInScreen())
                return true;
        ...
        {
            if (!SoftMaskingEnabled() || !graphic || !graphic.canvas)
                return;

            if (FrameCache.TryGet(this, nameof(RenderSoftMaskBuffer), out bool _))
                return;
            FrameCache.Set(this, nameof(RenderSoftMaskBuffer), true);

            if (!isDirty)
                return;
            if (!renderEveryFrame && _hasSoftMaskBufferDrawn && !_hasResolutionChanged)
                return;
            isDirty = false;
